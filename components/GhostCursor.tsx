...
// Ghost cursor with low-opacity deep blue-white color
...
const UNPREMULTIPLY_VERTEX_SHADER = `
  varying vec2 vUv;
  void main(){
    vUv = uv;
    gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
...
const UNPREMULTIPLY_FRAGMENT_SHADER = `
  uniform sampler2D tDiffuse;
  varying vec2 vUv;
  void main(){
...
const GhostCursor: React.FC<GhostCursorProps> = ({
  ...
  brightness = 0.95,
  color = "#6b8cff", // deep blue with a tiny shift toward white
  ...
  const fadeOpacityRef = useRef(0.35); // ~35% base opacity
  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        iOpacity: { value: fadeOpacityRef.current },
        iScale: { value: 1.0 },
        iBaseColor: {
  ...
  fadeOpacityRef.current = 0.35; // reset to ~35% when moving
      } else {
        ...
        if (dt > fadeDelay) {
          ...
          fadeOpacityRef.current = Math.max(0.0, 0.35 * (1 - k));
        }
      }
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      const x = THREE.MathUtils.clamp(e.clientX / viewportWidth, 0, 1);
      const y = THREE.MathUtils.clamp(1 - e.clientY / viewportHeight, 0, 1);
